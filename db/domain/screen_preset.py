...
def create_screen_preset(name: str, command: str, value_desc: str) -> db.DBResultDTO:
    try:
        conn = db.get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO terminal_screen_preset (name, command, value_desc) VALUES (?, ?, ?)", (name, command, value_desc))
        conn.commit()
...
def get_screen_preset_by_id(id: int) -> db.DBResultDTO:
    try:
        conn = db.get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM terminal_screen_preset WHERE id = ?", (id,))
...
def delete_screen_preset(id: int) -> db.DBResultDTO:
    ...
    try:
        conn = db.get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM terminal_screen_preset WHERE id = ?", (id,))
        conn.commit()
        db.close_db_connection(conn)
...
def update_screen_preset(id: int, name: str, command: str, value_desc: str) -> db.DBResultDTO:
    screen_preset_result = get_screen_preset_by_id(id)
    if not screen_preset_result.success:
        return screen_preset_result
    ...
    try:
        conn = db.get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE terminal_screen_preset SET name = ?, command = ?, value_desc = ? WHERE id = ?", (name, command, value_desc, id))
        conn.commit()
        db.close_db_connection(conn)
