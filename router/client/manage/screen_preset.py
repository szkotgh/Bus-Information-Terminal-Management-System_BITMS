...
def create():
    ...
    value1_desc = request.json.get('value1_desc', '')
    value2_desc = request.json.get('value2_desc', '')
    value3_desc = request.json.get('value3_desc', '')
    value4_desc = request.json.get('value4_desc', '')
    if not name or not command:
        return utils.ResultDTO(code=400, message="누락된 값을 확인하십시오.").to_response()

    create_result = db_screen_preset.create_screen_preset(
...
def update():
    ...
    update_result = db_screen_preset.update_screen_preset(
        id, name, command, value1_desc, value2_desc, value3_desc, value4_desc)
