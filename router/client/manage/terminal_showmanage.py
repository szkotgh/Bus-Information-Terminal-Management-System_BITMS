...
bp = Blueprint('terminal_showmanage', __name__, url_prefix='/terminal_showmanage')
...
def create():
    ...
    try:
        ...
        screen_preset_id = int(screen_preset_id) if screen_preset_id is not None else None
    ...
    if not terminal_id or not screen_preset_id or is_active is None or not order_id or not desc or not value:
        return utils.ResultDTO(code=400, message='누락된 값을 확인하십시오.').to_response()
    
    create_result = db_screen_show.create_screen_show(terminal_id, screen_preset_id, is_active, order_id, desc, value)
...
def update():
    ...
    update_result = db_screen_show.update_screen_show(screen_show_id, terminal_id, screen_preset_id, is_active, order_id, desc, value)
