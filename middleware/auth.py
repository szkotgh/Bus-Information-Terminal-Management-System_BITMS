...
def terminal_auth_token_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'Authorization' not in request.headers:
            return utils.ResultDTO(code=401, message='Authorization header is missing.').to_response()
        
        auth_token = utils.extract_bearer_token(request.headers['Authorization'])
        if not auth_token:
            return utils.ResultDTO(code=401, message='Bearer token is invalid.').to_response()
        
        terminal_result = db_terminal.get_terminal_by_auth_token(auth_token)
        if not terminal_result.success:
            return utils.ResultDTO(code=401, message=terminal_result.message).to_response()
        
        if not terminal_result.data['status'] == constants.STATUS_ACTIVE:
            return utils.ResultDTO(code=401, message="Status is not active.").to_response()
